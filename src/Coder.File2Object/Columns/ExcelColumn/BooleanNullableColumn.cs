...
{
    public class BooleanNullableColumn<TEntity> : Column<TEntity, ICell, bool?>
    {
        private readonly string _trueStrExpress;

        public BooleanNullableColumn(string name, Expression<Func<TEntity, bool?>> action, string trueStrExpress, bool isRequire = false) : base(name, action, isRequire)
        {
